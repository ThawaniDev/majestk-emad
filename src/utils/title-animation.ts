...
export const splitAnimation = (): void => {
  ...
  elements.forEach((el) => {
    ...
    // Initialize SplitText: For RTL (Arabic), avoid splitting by chars
    ...
    // Target for animation: words for RTL, chars for LTR
    const target = isRtl ? split.words : split.chars;
